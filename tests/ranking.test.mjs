...
import { createRanking, escapeHtml, SUPABASE_URL } from '../dist/ranking.js';
import { RANKING_GAME, BOARDS, boardIdFor } from '../dist/ranking-boards.js';
...
test('every circuit has a board whose minimum lap matches the current layout', () => {
  assert.deepEqual(circuits.map(boardIdFor), BOARDS.map(b => b.id));
  for (const [i, circuit] of circuits.entries()) {
    // A lap cannot beat 85% of the track length (allowing for corner cutting) at the 96 m/s speed cap.
    assert.equal(BOARDS[i].min, Math.floor(buildTrack(circuit).length * .85 / 96 * 1000), `${circuit.id}: update dist/ranking-boards.js and supabase/ranking-boards.sql`);
    assert.equal(BOARDS[i].name, circuit.name);
  }
});